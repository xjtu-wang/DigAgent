  ...
  return (
    <div className="flex h-screen overflow-hidden bg-white text-slate-900">
      ...
      <div className="flex min-w-0 flex-1 overflow-hidden">
        ...
        <InspectorDrawer open={inspectorOpen} activityEvents={controller.activityEvents} catalog={catalog} currentTab={inspectorTab} currentTurn={controller.currentTurn} cveQuery={controller.cveQuery} cveResults={controller.cveResults} cveStatus={controller.cveStatus} messages={controller.messages} onChangeTab={setInspectorTab} onClose={() => setInspectorOpen(false)} onRuntimeDraftChange={controller.setRuntimeDraft} onSearchCve={() => void controller.searchCve()} onSelectNode={controller.setSelectedNodeId} onSelectTurn={controller.selectTurn} onSyncCve={() => void controller.syncCve()} planGraph={controller.planGraph} runtimeDraft={controller.runtimeDraft} selectedGraphNode={controller.selectedGraphNode} selectedNodeId={controller.selectedNodeId} session={controller.session} setCveQuery={controller.setCveQuery} turns={controller.turns} workflowPreferences={settings.workflowPreferences} />
      </div>
    </div>
  );
}
